var editor = null;
var ptagsEditor = null;
var xhtmlEditor = null;
(function() {
    ...
    Event.onDOMReady(function() {
        ...
        var ptagsEditorHasRendered = false;
        ptagsEditor = new YAHOO.widget.Editor('ptags_editor', {
            dompath: true,
            nodeChangeDelay: false,
            removeLineBreaks: true,
            animate: true,
        ...
        var xhtmlEditorHasRendered = false;
        xhtmlEditor = new YAHOO.widget.Editor('xhtml_editor', {
            dompath: true,
            nodeChangeDelay: false,
            removeLineBreaks: true,
            animate: true,
        ...
        var preTestInitHasFinished = function() {
            return editorHasRendered && 
              ptagsEditorHasRendered &&
              xhtmlEditorHasRendered;
        };
        
        // use this function to write debugging messages to the test runner console.
        ...
        var runTestSuite = function() { 
            // We have to wait until the all windows render before we can start testing it..
            if (preTestInitHasFinished) {
              Suite.add( new Tool.TestCase({
                  ...
                  test_forecolor_font_el: function() {
                      ...
                      Assert.areEqual('b', el.tagName.toLowerCase(), 'Elements do not match');
                  },
                  ...
                  test_starting_with_an_empty_textarea_should_result_in_an_emtpy_textarea: function() {
                      editor.setEditorHTML('');
                      editor.saveHTML();
                      
                      var textareaContents = document.getElementById('editor').value;
                      Assert.areEqual('', textareaContents, 'Expected for the text area to contain nothing.');                                    
                  },
                  test_ptags_starting_with_empty_textarea_should_contain_only_an_empty_paragraph: function() {
                      var body = ptagsEditor._getDoc().body;
                      var root = body.firstChild;
                      Assert.areEqual('p', root.tagName.toLowerCase(), 'Expected paragraph element.');                  
                  },
                  test_ptags_typing_enter_key_should_not_result_in_a_line_containg_a_single_space_that_is_selected: function() {
                      var body = ptagsEditor._getDoc().body;                      
                      YAHOO.util.UserAction.click(body);                                         
                      YAHOO.util.UserAction.keypress(body, { charCode: 13 });
                              
                      var root = body.firstChild;
                      Assert.isNotNull(root, 'Editor failed to contain text after hitting the enter key.');                     
                      Assert.areEqual('p', root.tagName.toLowerCase(), 'Expected paragraph element.');                  
                  },
                  test_xhtml_generates_correct_break_tags: function() {
                      var textareaContents = document.getElementById('xhtml_editor').value;
                      Assert.areEqual('', textareaContents, 'Expected self closing br element.');                                  
                  
                      var body = xhtmlEditor._getDoc().body;                      
                      YAHOO.util.UserAction.click(body);                                         
                      YAHOO.util.UserAction.keypress(body, { charCode: 13 });
                                           
                      xhtmlEditor.saveHTML();
                      
                      textareaContents = document.getElementById('xhtml_editor').value;
                      Assert.areEqual('<br />', textareaContents, 'Expected self closing br element.');                  
                  }                           
              })); 
        ...
        ptagsEditor.on('windowRender', function() {
            ptagsEditorHasRendered = true;
            runTestSuite();
        });
        
        xhtmlEditor.on('windowRender', function() {
            xhtmlEditorHasRendered = true;
            runTestSuite();
        });
    }); 
})();
